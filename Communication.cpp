...
static int decode_message(uint8_t in_buffer[INPUT_BUFFER_SIZE], handdata_t &data_struct) {
    pb_istream_t istream = pb_istream_from_buffer(in_buffer, INPUT_BUFFER_SIZE);
...
void Decode_Task(void *pvParameters) {
    ...
    // Set up UART interrupt, data buffer, etc
    ...
    communication_stream_buf = xStreamBufferCreate(handtracking_HandData_size, 0);
    ...
    while(1) {
        ...
        auto status = decode_message(reinterpret_cast<uint8_t *>(input_data), handdata_temp);
        if(!status) {
